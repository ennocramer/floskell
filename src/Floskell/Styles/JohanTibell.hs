...
dependOrNewline :: Printer t ()
                ...
                -> Printer t ()
dependOrNewline left right f = do
    renderDependent `fitsOnOneLineOr` do
        left
        newline
        (f right)
  where
    renderDependent = depend left (f right)
...
guardedRhs (GuardedRhs _ stmts e) = do
    ...
    (do
        ...
        swingIt
  where
    swingIt = swing (write " " >> rhsSeparator >> write " ") (pretty e)
...
exp (App _ op a) = do
    (spaced (map pretty (f : args))) `fitsOnOneLineOr` do
        pretty f
        newline
...
exp (List _ es) = p `fitsOnOneLineOr`
    brackets (prefixedLined "," (map (depend space . pretty) es))
  where
...
exp (Let _ binds e) = depend (write "let ")
                             (do
                                  pretty binds
                                  newline
                                  indented (-4)
...
context ctx@(CxTuple _ asserts) =
    (parens (inter (comma >> space) (map pretty asserts))) `fitsOnOneLineOr`
        prettyNoExt ctx
context ctx = prettyNoExt ctx

unboxParens :: Printer s a -> Printer s a
unboxParens p = depend (write "(# ")
                       (do
                            v <- p
                            write " #)"
                            return v)
...
typ (TyTuple _ Unboxed types) = unboxParens $ inter (write ", ") $ map pretty
                                                                       types
...
decl (TypeSig _ names ty') = do
  ...
  where
    ...
    collapseFaps e = [ e ]
    prettyTy ty = do
        small <- isSmall (pretty ty)
        if small
            then pretty ty
            else case collapseFaps ty of
                [] -> pretty ty
                tys -> prefixedLined "-> " (map pretty tys)
decl (PatBind _ pat rhs' mbinds) = withCaseContext False $ do
    pretty pat
    pretty rhs'
    forM_ mbinds bindingGroup
