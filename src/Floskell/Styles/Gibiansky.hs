...
import           Control.Monad.State.Strict     hiding ( State, forM_
                                                       , sequence_, state )
      ...
      where
        ...
        groupComments nextLine accum (comment@(Comment multiline srcSpan str) : comments)
            ...
            | beginningOfUnprocessed str =
                  let (unprocessedLines, postUnprocessed) = span unprocessed
                                                                 comments
                      (endingLine, remLines) = case postUnprocessed of
                          x : xs -> ([ x ], xs)
                          [] -> ([], [])
                      separateCommentGroups =
                          comment : unprocessedLines ++ endingLine
                  in
                      currentGroupAsList ++ map (: []) separateCommentGroups ++
                          groupComments Nothing [] remLines
            | isNothing nextLine || Just (srcSpanStartLine srcSpan) ==
                  nextLine = groupComments nextLine' (comment : accum) comments
            | otherwise = currentGroupAsList ++
                  groupComments (Just $ srcSpanStartLine srcSpan + 1)
                                [ comment ]
                                comments
          where
            separateCommentCondition = or [ multiline
                                          , isWhitespace str
                                          , "  " `isPrefixOf` str
                                          , " >" `isPrefixOf` str
                                          ]
            useAsSeparateCommentGroup = currentGroupAsList ++
                [ comment ] : groupComments nextLine' [] comments
            nextCommentStartLine = srcSpanStartLine $ commentSrcSpan $ head comments
            currentGroupAsList
                | null accum = []
                | otherwise = [ reverse accum ]
            nextLine' = case nextLine of
                Just x -> Just (x + 1)
        ...
        mergeGroup comments =
            let firstSrcSpan = commentSrcSpan $ head comments
                firstLine = srcSpanStartLine firstSrcSpan
                firstCol = srcSpanStartColumn firstSrcSpan

                columnDelta = firstCol - currentColumn
                maxStartColumn =
                    maximum (map (srcSpanStartColumn . commentSrcSpan) comments)

                lineLen = fromIntegral (configMaxColumns config) -
                    maxStartColumn - commentLen + columnDelta
                content = breakCommentLines lineLen $
                    unlines (map commentContent comments)
                srcSpanLines = map (firstLine +) [0 .. length content - 1]
                srcSpans =
                    map (\linum ->
                             firstSrcSpan { srcSpanStartLine = linum
                                          , srcSpanEndLine = linum
                                          , srcSpanStartColumn = maxStartColumn
                                          })
                        srcSpanLines
            in
                zipWith (Comment False) srcSpans content
...
-- If there's no way to do this formatting, just give up
    ...
    | length (lines str) ==
          1 && length str <= maxLen = [ dropTrailingNewlines str ]
  ...
  where
    ...
    unfolder ws = Just $ go maxLen [] ws
      where
        ...
        go remainingLen taken remainingWords = case remainingWords of
...
attemptSingleLine single multiple = do
    forceSingleLine <- gets (gibianskyForceSingleLine . psUserState)
    if forceSingleLine then single else cut (trySingle <|> multiple)
  where
    trySingle = do
...
pat (PRec _ name fields) =
    recUpdateExpr fields (pretty name) (map prettyCommentCallbacks fields)
pat p = prettyNoExt p
...
imp ImportDecl{..} = do
    write "import "
    write $ if importQualified then "qualified " else "          "
    pretty importModule
            ...
            case specs of
                ...
                x : xs -> do
                    ...
                    forM_ xs $ \spec -> do
                        ...
                        if col + len > maxColumns then newline else space
...
prettyColLength x = fst <$> sandbox (do
                                         ...
                                         col' <- getColumn
                                         return $
                                             fromIntegral $ max (col' - col) 0)

-- | Format contexts with spaces and commas between class constraints.
context :: Extend Context
context (CxTuple _ asserts) = parens $ inter (comma >> space) $ map pretty
                                                                    asserts
...
multiIfExpr (MultiIf _ alts) = withCaseContext True $
    depend (write "if ") $
...
appExpr app@(App _ f x) = do
  ...
  where
    ...
    collectArgs (App _ g y) = let (fun, args) = collectArgs g
                              in
                                  (fun, y : args)
    collectArgs nonApp = (nonApp, [])

    separateArgs :: Exp NodeInfo -> Printer State ()
    separateArgs expr = let (fun, args) = collectArgs expr
                        in
                            keepingColumn $ do
                                pretty fun
                                newline
...
listExpr (List _ els) = attemptSingleLine (singleLineList els)
                                          (multiLineList els)
...
applicativeExpr exp@InfixApp{} = case applicativeArgs of
    Just (first : second : rest) ->
        attemptSingleLine (singleLine first second rest)
                          (multiLine first second rest)
    _ -> prettyNoExt exp
  where
    singleLine :: Exp NodeInfo
               -> Exp NodeInfo
               -> [Exp NodeInfo]
               -> Printer State ()
    singleLine first second rest = spaced [ pretty first
                                          , write "<$>"
                                          , pretty second
                                          , write "<*>"
                                          , inter (write " <*> ") $ map pretty
                                                                        rest
                                          ]

    multiLine :: Exp NodeInfo
              -> Exp NodeInfo
              -> [Exp NodeInfo]
              -> Printer State ()
...
opExpr expr@(InfixApp _ left op right) = keepingColumn $ do
  ...
  where
    ...
    userSpecified deltaLeft deltaRight = do
        ...
        if deltaLeft == 0 then space else replicateM_ deltaLeft newline

        pretty op

        if deltaRight == 0 then space else replicateM_ deltaRight newline
    ...
    collectOpArguments op expr'@(InfixApp _ left' op' right')
        | void op == void op' =
              collectOpArguments op left' ++ collectOpArguments op right'
        | otherwise = [ expr' ]
...
ifExpr (If _ cond thenExpr elseExpr) = depend (write "if") $ do
    space
    pretty cond
    newline
    write "then "
...
writeCaseAlts alts = do
    ...
    withCaseContext True $ indented indentSpaces $ do
        prettyPr <- if allSingle
                    then do
                        maxPatLen <- maximum <$> mapM (patternLen . altPattern)
                                                      alts
  ...
  where
    ...
    patternLen pat = fromIntegral <$> fst
                                  <$> sandbox (do
                                                   col <- getNextColumn
                                                   pretty pat
                                                   col' <- getNextColumn
...
prettyCommentCallbacks :: Pretty ast
                       => ast NodeInfo
                       -> (ComInfoLocation -> Printer s ())
                       -> Printer s ()
prettyCommentCallbacks a f = do
    st <- get
    case st of
...
recUpdateExpr :: Foldable f
              => [f NodeInfo]
              -> Printer State ()
              -> [(ComInfoLocation -> Printer State ())
                  -> Printer State ()]
              -> Printer State ()
...
rhss (UnGuardedRhs rhsLoc exp) = do
    ...
    let exp'
            | lineBreakAfterRhs rhsLoc exp = indented indentSpaces $ do
                  newline
                  pretty exp
            | letBind = depend space (pretty exp)
    ...
    flip onSeparateLines' rs $ \a@(GuardedRhs rhsLoc stmts exp) -> do
        ...
        if manyStmts
            then do
                ...
                remainder
            else depend (write "| ") $ writeStmts >> remainder
...
decls (FunBind _ matches) = flip onSeparateLines' matches $ \match -> do
    printComments Before match
    (writeName, pat, rhs, mbinds) <- case match of
                                         Match _ name pat rhs mbinds ->
                                             return ( pretty name
                                                    , pat
                                                    , rhs
                                                    , mbinds
        ...
        depend (maybeCtx ctx) $
            depend (pretty dhead >> space) $
                depend (unless (null fundeps)
                               (write "| " >>
                                    commas (map pretty fundeps) >> space)) $
...
instRule (IRule _ mvarbinds mctx ihead) = do
    case mvarbinds of
        Nothing -> return ()
...
fixSpans info =
    ...
    in
        if length points > 1 && lastPtEndLoc == infoEndLoc && invalidLastPt
        then info { nodeInfoSpan = infoSpan { srcInfoSpan = setEnd srcSpan
                                                                   prevPtEnd
                                            }
                  }
        else info
  where
    setEnd (SrcSpan fname startL startC _ _) (endL, endC) =
        SrcSpan fname startL startC endL endC
...
condecls (ConDecl _ name bangty) = depend (space >> pretty name) $
    forM_ bangty $ \ty -> space >> pretty ty
condecls decl@(RecDecl _ name fields) =
    if hasComments decl then multiRec else attemptSingleLine singleRec multiRec
  where
