...
data Commented =
       Commented
         { singleField :: Int -- with a comment
         }

data LongTypeSig =
       LongTypeSig
         { field :: (IsString a, Monad m) => (ByteString -> ByteString) -> ByteString -> a -> m () }
```
...
-- comment
defaultExtensions = [ e
                    | e@EnableExtension{} <- knownExtensions ] \\ map EnableExtension badExtensions
```
...
data X =
       X
         ...
         -- the field but continues onto the next line, it aligns exactly with the field name.
         , field4 :: Char-- ^ This is a long comment which starts on the following line from from
                         -- the field, lines continue at the sme column.
         }
```
...
``` haskell
-- This is some random comment. | Main entry point.
